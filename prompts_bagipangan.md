...
Stack:
- Backend: Laravel (PHP 8.2 atau terbaru), Supabase PostgreSQL (monolithic database)
...
- Write an E2E test that navigates to /donations/{id} and asserts the title, status badge, and donor info are visible.
- Assert "Klaim Donasi" button only appears when logged in as a Penerima with an available donation.
...
Stack:
- Backend: Laravel (PHP 8.2 atau terbaru), Supabase PostgreSQL (monolithic database)
- Frontend: Next.js (SSR + CSR), Tailwind CSS, recharts
- Web Server: Apache
- Testing: Laravel Dusk (Automated E2E)
...
### Testing (Laravel Dusk)
- Write an E2E test that logs in as admin, navigates to /admin/dashboard, and asserts summary cards and charts are rendered.
- Test date range filter: set date_from and date_to, assert data updates.
...
### Testing (Laravel Dusk)
- Write an E2E test that navigates to /donations, types a keyword, and asserts matching results appear.
- Test category filter: select a category and assert only matching cards are shown.
...
### Testing (Laravel Dusk)
- Write an E2E test that logs in as admin, sets a date range, clicks Export CSV, and asserts the download is triggered.
- Assert that an audit_logs entry is created after export.
...
Stack:
- Backend: Laravel (PHP 8.2 atau terbaru), Supabase PostgreSQL (monolithic database)
- Frontend: Next.js (SSR + CSR), Tailwind CSS, react-leaflet, Leaflet.js
...
### Testing (Laravel Dusk)
- Write an E2E test that navigates to /donations/map and asserts the map container is rendered.
- Assert that at least one marker is visible when available donations with coordinates exist.
...
### JavaScript Integration (Next.js)
...
export async function chatWithBagiPanganAI(userMessage, conversationHistory, userContext) {
